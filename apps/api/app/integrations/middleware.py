...
class MiddlewareAdapter:
    ...
    @staticmethod
    def headers(body: bytes, idempotency_key: str, timestamp: str, nonce: str) -> dict[str, str]:
        digest = hashlib.sha256(body).hexdigest()
        values = (
            VERSION, "POST", PATH, timestamp, nonce, settings.middleware_service_identity,
            settings.middleware_audience, settings.app_env, settings.middleware_scope,
            idempotency_key, digest,
        )
        if any(not value or "\n" in value or "\r" in value for value in values):
            raise OdooDeliveryError("MIDDLEWARE_SIGNING_CONFIGURATION_INVALID", terminal=True)
        signature = hmac.new(
    ...
    async def deliver(self, event: object) -> OdooResult:
        ...
        try:
            ...
            raise OdooDeliveryError(f"MIDDLEWARE_HTTP_{exc.response.status_code}", terminal=terminal) from exc
        if acknowledgement.get("event_id") != envelope["event_id"] or acknowledgement.get("status") not in {"queued", "delivered", "replayed"}:
            raise OdooDeliveryError("MIDDLEWARE_INVALID_ACK", terminal=True)
        external_id = acknowledgement.get("odoo_record_id") or 0
        return OdooResult(int(external_id), str(acknowledgement.get("odoo_model", "middleware.pending")))
