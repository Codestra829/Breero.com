...
async def provider_health(_: User = Depends(require_roles(UserRole.finance, UserRole.admin))):
    return {
        ...
        "odoo": {"configured": bool(settings.odoo_url and settings.odoo_database and settings.odoo_username and settings.odoo_api_key)},
        "geocoder": {"configured": bool(settings.geocoding_api_key), "provider": settings.geocoding_provider},
        "payout": {"configured": bool(settings.payout_provider)},
    }


@router.get("/failures")
async def failures(session: AsyncSession = Depends(get_db),
                   _: User = Depends(require_roles(UserRole.finance, UserRole.admin))):
    return list((await session.scalars(select(IntegrationEvent).where(
        IntegrationEvent.status.in_([EventStatus.FAILED, EventStatus.DEAD_LETTER])
    ).order_by(IntegrationEvent.created_at.desc()).limit(200))).all())


@router.post("/events/{event_id}/retry")
async def retry_event(event_id: uuid.UUID, session: AsyncSession = Depends(get_db),
