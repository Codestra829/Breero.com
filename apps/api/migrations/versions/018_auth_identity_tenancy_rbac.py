"""identity links, tenant-aware access assignments and permission overrides
...
def upgrade():
    op.create_table(
        ...
        sa.UniqueConstraint("brand_key", "issuer", "subject", name="uq_identity_link_subject"),
        sa.UniqueConstraint("brand_key", "issuer", "user_id", name="uq_identity_link_user_issuer"),
    )
    ...
    op.create_table(
        ...
        sa.Column("tenant_scope", sa.String(32), nullable=False, server_default="brand"),
        sa.Column(
            "vendor_id",
            postgresql.UUID(as_uuid=True),
    ...
    op.create_index("ix_access_assignments_user_id", "access_assignments", ["user_id"])
    op.create_index("ix_access_assignments_vendor_id", "access_assignments", ["vendor_id"])
    op.create_index("ix_access_assignments_department", "access_assignments", ["department"])
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("role_key", "permission", name="uq_role_permission"),
    )
    op.create_index("ix_role_permissions_role_key", "role_permissions", ["role_key"])
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.UniqueConstraint("user_id", "brand_key", "permission", name="uq_user_permission"),
    )
