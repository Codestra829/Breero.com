...
export function AuthForm({ mode }: { mode: Mode }) {
  ...
  return <form action={submit} aria-busy={state === "loading"}>
    ...
    {mode === "login" && <div className="auth-options"><Checkbox label="Keep me signed in for this browser session" /><a href="/forgot-password">Forgot password?</a></div>}
    ...
    {state === "error" && <p className="auth-message auth-error" role="alert">{mode === "login" ? "Those details don’t match. Check them and try again." : message || "We couldn’t complete that request. Please try again."}</p>}
