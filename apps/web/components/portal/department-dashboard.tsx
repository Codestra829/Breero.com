...
export function DepartmentDashboard({ department, eyebrow, title, description, modules }: DepartmentDashboardProps) {
  ...
  useEffect(() => {
    if (context && !canAccessDepartment(context, department)) {
  ...
  if (!canAccessDepartment(context, department)) return <div className="shell market-section"><LoadingState label="Routing to your authorized workspace"/></div>;
