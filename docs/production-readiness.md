...
- Fresh database and prior `005_booking_integrations` database both migrated to
  `007_finance_integrations`; destructive column/table drift check passed.
- Custom-format `pg_dump` was listed and restored into an isolated database; restored
  revision was `007_finance_integrations` with 33 public tables.
...
3. Run the one-shot `migrate` profile and verify revision `007_finance_integrations`.
