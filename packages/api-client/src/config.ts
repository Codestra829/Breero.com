...
export function readPublicApiConfig(env: Record<string, string | undefined>): PublicApiConfig {
  const apiBaseUrl = env.NEXT_PUBLIC_API_BASE_URL ?? (env.NODE_ENV === "production" ? "https://api.breero.com/api/v1" : "http://localhost:8000/api/v1");
  ...
  if (env.NODE_ENV === "production" && env.NEXT_PUBLIC_E2E_ALLOW_MOCK !== "1" && /localhost|127\.0\.0\.1/.test(apiBaseUrl)) throw new Error("Production requires a non-local NEXT_PUBLIC_API_BASE_URL");
