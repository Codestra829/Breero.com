...
describe("mock booking journey", () => {
  it("supports the public funnel through authoritative payment state", async () => {
    ...
    const payment = await api.payments.createIntent({ booking_id: booking.id, amount_minor: 8900, currency: "eur" }, "payment-test-100");
    expect({ service: service.slug, serviceable: address.serviceable, booking: booking.status, payment: payment.status }).toEqual({ service: "cleaning", serviceable: true, booking: "PENDING_PAYMENT", payment: "REQUIRES_ACTION" });
  });
});
